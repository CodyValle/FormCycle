     ...
     */
        ...
        case city:
          if string.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceAndNewlineCharacterSet()).characters.count == 0
          {
            ...
            prospectiveText.characters.count <= 11
        ...
        if string.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceAndNewlineCharacterSet()).characters.count == 0
        {
          if currentText.characters.count == 0 {
            ...
            return currentText.characters.last != " "
				...
				/* Allow only upper-case letters in this field, and must have only 2 characters. */
				case tagNumber:
					return prospectiveText.containsOnlyCharactersIn("0123456789") && prospectiveText.characters.count <= 2
  ...
  +-------------------------------------------------------------------------------------+*/
    ...
    if segue.identifier == "moveToAddServices" {
      ...
      newOrderTextFieldStruct.custid = ""
		...
		/* check if the user pressed the next button on the cust info page. */
...
/*+------------------------- shouldPerformSegueWithIdentifier ---------------------------+
   ...
   */
    ...
    if newOrderTextFieldStruct.loginPage {
      ...
      else if phone.text?.utf16.count < 10// || phone.text?.utf16.count != 10 || phone.text?.utf16.count != 11 /* Checks phone number restrictions */
			{
				let refreshAlert = UIAlertController(title: "Incorrect Number of Digits for Phone Number", message: "Requires 7, 10. or 11 digits (e.g. XXX-XXX-XXXX), Try Again", preferredStyle: UIAlertControllerStyle.Alert)
